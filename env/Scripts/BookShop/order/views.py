	...
	if request.user.is_authenticated:
		...
		if request.method == 'POST':
			if form.is_valid():
				...
				messages.error(request, "Veuillez remplir vos informations correctement.")
...
def order_list(request):
	my_order = History.objects.filter(user_id = request.user.id).order_by('-created')
	#paginator = Paginator(my_order, 5)
	#page = request.GET.get('page')
	#myorder = paginator.get_page(page)

	return render(request, 'order/list.html', {"myorder": my_order})
